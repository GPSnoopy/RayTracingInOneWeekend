...
impl Material for Dielectric {
    fn scatter(&self, ray: &Ray, record: &HitRecord, random: &mut Random) -> Option<MaterialRay> {
        ...
        if uniform(random) < reflect_prob
            { Some(MaterialRay::new(Ray::new(record.point, reflect(ray.direction, record.normal)), Vec3::one())) } else
            { Some(MaterialRay::new(Ray::new(record.point, refracted.unwrap()), Vec3::one())) }
    }
}
